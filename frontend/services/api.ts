...
) => CompanyFactory__factory.connect(CompanyFactoryAddress, signerOrProvider);
...
export const getExchangeRate = async () => {
  const exchangeRate = await cc.price("FTM", ["USD"]);
...
export const PayEmployees = async (
  ...
  const instance = fetchCompanyContract(address, signer);
  await instance.payEmployee({ value: tokens(amount) });
};
